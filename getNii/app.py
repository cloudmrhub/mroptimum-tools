...
#             }
    ...
    for data in ledge:
        ...
        target_key = f"{file_key}_{os.path.basename(file)}"
        s3_client.put_object(Body=file, Bucket=bucket_name, Key=target_key)
        # Generate a pre-signed URL for the uploaded .nii file
        url = s3_client.generate_presigned_url('get_object',Params={'Bucket': target_bucket, 'Key': target_key},
